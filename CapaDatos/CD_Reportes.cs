                ...
                try
                {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader()) {
                        while (dr.Read()) {
                            ...
                            {
                                RucTienda = dr["Ruc Sucursal"].ToString(),
                                NombreTienda = dr["Nombre Sucursal"].ToString(),
                                DireccionTienda = dr["Direccion Sucursal"].ToString(),
                                ...
                                StockenTienda = dr["Stock en Sucursal"].ToString(),
                                PrecioCompra = (Convert.ToDecimal(dr["Precio Compra"].ToString(),new CultureInfo("es-PE"))/100).ToString("N", formato),
                                PrecioVenta = (Convert.ToDecimal(dr["Precio Venta"].ToString(),new CultureInfo("es-PE"))/100).ToString("N", formato)
                            });
                        }
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            lista.Add(new ReporteVenta()
                            {
                                ...
                                TotalVenta = (Convert.ToDecimal(dr["Total Venta"].ToString(), new CultureInfo("es-PE"))/100).ToString("N", formato)
                            });
                        }
